...
epoch=1;
p = 0.5;            % probability taken into account for updating weights from input to hidden layer

while epoch==1 || J2(epoch-1)>=250000 % low value is 250000 from previous
    ...
    for q = 1:4000
        ...
        % Updating weights from input to hidden layer at a probability p
        w_ij=w_ij+dw_ij;
        nos=p*200*785;
        a=zeros(200,785);
        index = randperm(numel(w_jk));
        a(index(1:nos)) = 1;
        
        % Updating weights from hidden to output layer as it is
        w_jk=w_jk+a.*dw_jk;
    end
    epoch=epoch+1;
end
